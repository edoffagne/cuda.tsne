...
def _segment_sort_transposed_distances_get_knns(num_of_neighbours, distances_on_gpu, number_of_sorts,
    ...
    for i in np.arange(1, p.shape[0]):
        delta_n = p[i] - p[i - 1]
        keys = numba.types.float32(np.ascontiguousarray(distances_on_gpu.copy_to_host()[:, p[i - 1]:p[i]].transpose().reshape(m * delta_n)))
        values = numba.types.uint32(np.ascontiguousarray(np.tile(np.arange(m), (delta_n, 1)).reshape(m * delta_n)))
        segments = numba.types.uint32(np.ascontiguousarray(np.arange(m, m * delta_n, m)))
        #segments.sort()
...
def calculate_knn_distances(template_features_sparse_clean, perplexity=100, mem_usage=0.9, verbose=True):
    ...
    for iter in np.arange(number_of_iters):
        ...
        k = len(closest_indices[1])
    return closest_indices.flatten(), np.sqrt(np.abs(closest_distances)).flatten(), k
